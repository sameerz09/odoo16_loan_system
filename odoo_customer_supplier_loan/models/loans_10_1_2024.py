...
class EmployeeLoanDetails(models.Model):
    ...
    loan_type = fields.Many2one(
        ...
        required=True,
        readonly=True,
        states={'draft': [('readonly', False)], 'applied': [('readonly', False)]}
    )
  ...
  #  states = {'paid': [('readonly', True)], 'disburse': [('readonly', True)]}
    ...
    loan_prog = fields.Selection(
        selection=[('trial', 'Trial'),
                   ...
                   ('seasonal', 'Seasonal'),
                   ('vegetable_chains', 'Vegetable Chains'),
                   ],
        string='Loan Program',
    ...
    branch = fields.Selection([('HQ', 'Head Quarter'),
                   ('BR', 'Branch'),
                               ],
        required='True',
        default='HQ',
    ...
    def _get_intrest(self):
        for sheet in self:
            ...
            elif sheet.loan_prog == 'vegetable_chain' and sheet.principal_amount <= 10000:
                loan_t = self.env['partner.loan.type'].search([('code', '=', 'vc01')])
                sheet.update({
                    'loan_type': loan_t,
                    'int_rate': 500,
                })

            elif sheet.loan_prog == 'vegetable_chain' and sheet.principal_amount > 10000 and sheet.principal_amount <= 30000:
                loan_t = self.env['partner.loan.type'].search([('code', '=', 'vc02')])
                sheet.update({
                    'loan_type': loan_t,
                    'int_rate': 1500,
                })

            elif sheet.loan_prog == 'vegetable_chain' and sheet.principal_amount > 30000 and sheet.principal_amount <= 50000:
                loan_t = self.env['partner.loan.type'].search([('code', '=', 'vc03')])
                sheet.update({
                    'loan_type': loan_t,
                    'int_rate': 3000,
                })

            elif sheet.loan_prog == 'vegetable_chain' and sheet.principal_amount > 50000:
                loan_t = self.env['partner.loan.type'].search([('code', '=', 'vc04')])
                sheet.update({
                    'loan_type': loan_t,
                    'int_rate': 4000,
                })
