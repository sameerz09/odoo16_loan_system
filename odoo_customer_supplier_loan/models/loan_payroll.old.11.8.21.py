...
#         return result
    ...
    def _cal_amount_all(self):
        for rec in self:
            ...
            if rec.int_payable:
                if rec.interest_mode == 'flat':
                    rec.total_interest_amount = self.\
                        flat_rate_method(rec.principal_amount, rec.int_rate, rec.duration)
            ...
            rec.simulation = rec.principal_amount + rec.total_interest_amount
...
#                    else:
    ...
    @api.model
    def fees_rate_method(self, principal, rate, duration):
        return rate

    @api.model
    def sumation_rate_method(self, principal, rate, duration):
        return principal
