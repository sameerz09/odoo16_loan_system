...
class PartnerLoanInstallmentDetails(models.Model):
    ...
    def book_interest_new(self):
        ...
        for install in self:
            ...
            partner_id = address_id.id if address_id else False

            if not partner_id:
                raise UserError(_('Please configure Home Address on partner.'))
            ...
            if install.interest_amt > 0.0:
                credit_account_id = install.recievable_account_id.id
                debit_account_id = install.bank_account_id.id

                if not install.bank_journal_id:
                    raise UserError(_('Please configure Bank Journal.'))
                if not credit_account_id or not debit_account_id:
                    raise UserError(
                        _('Please configure Debit/Credit accounts for the Journal %s') % install.bank_journal_id.name)
                ...
                vals.update(int_move_id=move_id.id, date_from=timenow)
                install.write(vals)
        return True
    ...
    def action_pay_installment(self):
        ...
        for install in self:
            ...
            if install.interest_amt == 0.0:
                debit_account_id = install.bank_account_id.id
                credit_account_id = install.bank_account_id.id  # Replace with correct account logic
                if not credit_account_id or not debit_account_id:
                    raise UserError(
                        _('Please configure Debit/Credit accounts for the Journal %s') % install.bank_journal_id.name)
            ...
            if install.interest_amt > 0.0:
                ...
                move.update({'line_ids': [debit_line, credit_line, total_debit_line, total_credit_line]})
            else:
                move.update({'line_ids': [debit_line, credit_line]})

            move_id = move_pool.create(move)
            vals.update(state='paid', total_amt_move_id=move_id.id, date_from=timenow)
            install.write(vals)
            install.loan_id.compute_installments()
        return True
