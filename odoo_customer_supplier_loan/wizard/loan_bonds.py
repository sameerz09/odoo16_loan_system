...
class loanagesreceivables(models.TransientModel):
    _name = 'loan.ages.receivables'
    _description = 'Loan Age Receivables'
...
class loanagespayments(models.TransientModel):
        _name = 'loan.summery.payments'
        _description = 'Loans Summery Payments'
        ...
        def _print_report(self, data):
            records = self.env[data['model']].browse(data.get('ids', []))
            return self.env.ref('odoo_customer_supplier_loan.sum_paym_excel').report_action(records, data=data)
