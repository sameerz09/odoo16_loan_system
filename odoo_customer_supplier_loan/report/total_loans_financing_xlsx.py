...
class totalLoansFinancing(models.AbstractModel):
    _name = 'report.odoo_customer_supplier_loan.total_loans_financing_xlsx'
    _inherit = 'report.report_xlsx.abstract'
    _description = 'Total Loans Financing'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        #  sheet.set_column(0, 86, 25)
        sheet.merge_range('R1:AG1', 'ﺖﻗﺮﻳﺭ المقترضين', cell_format)
        ...
        sheet.write(row, col + 17, 'المبلغ الاجمالي بعد احتساب الفائدة/العمولة', cell_format)
        ...
        sheet.write(row, col + 31, 'العمر', cell_format)
        sheet.write(row, col + 32, 'رقم هوية المستفيد', cell_format)
        sheet.write(row, col + 33, 'اسم المستفيد', cell_format)
        # loaners = self.env['res.partner'].search([("active", "=", True), ("category.id", "=", 5)])
        loaners = self.env['partner.loan.details'].search(['|', '|', ("state", "=", 'disburse'),("state", "=", '1disburse'),("state", "=", '2disburse'),("date_disb", "!=", False)])
        ...
        for loaner in loaners:
            ...
            sheet.write(row, col + 31, loaner.partner_id.age, cell_format2)
            sheet.write(row, col + 32, loaner.partner_id.card_id, cell_format2)
