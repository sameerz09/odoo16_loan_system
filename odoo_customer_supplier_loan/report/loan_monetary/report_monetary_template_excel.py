   ...
   #     sheet.write(row, col + 29, 'رقم الهوية  جواز السفر الأول', bold)
    ...
    #    sheet.write(row, col + 32, ' نوع الضمانة المقدمة', bold)
       ...
       #     balance_amt = loaner.total_amount_due
            ...
            date_disb = loaner.actual_grant_date.strftime("%d/%m/%Y")
         ...
         #       d2 = datetime.datetime.strptime(str(com.payment_date), '%Y-%m-%d')
          ...
          #      d3 = (d2 -d1).days
                ...
                if com.payment_date:
                 ...
                 #   t1 = datetime.datetime.strptime(str(com.date_from), '%m-%Y-%d')
                  ...
                  if loaner.state != 'disburse' and unpaid_duration < 30:
                     cus_rank = 0
                  elif unpaid_duration < 30 and loaner.state == 'disbursed':
                    cus_rank = 1
            ...
            month = loaner.actual_grant_date.strftime("%m")
            year = loaner.actual_grant_date.strftime("%Y")
           ...
           # month = datetime.datetime.strptime(str(loaner.date_disb), '%Y-%m-%d').date()
            ...
            sheet.write(row, col + 6, company_id)

            sheet.write(row, col + 7, branch_id)
            sheet.write(row, col + 8, month)
            sheet.write(row, col + 9, year)
            ...
            sheet.write(row, col + 15, loaner.total_amount_paid)
            ...
            sheet.write(row, col + 26, loaner.int_rate)
            ...
            sheet.write(row, col + 46, 'DC')
            sheet.write(row, col + 47, '')
